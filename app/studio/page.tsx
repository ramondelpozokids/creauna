...
const translations = {
  es: {
    ...
    welcome: "Hola. Soy tu director de diseño personal. ¿Qué web te gustaría crear hoy?",
    ...
    improve: "Mejorar",
    thinking: "Refinando con mucho cuidado...",
    successUpdate: "Diseño actualizado",
  },
  en: {
    title: "CREAUNA Studio",
    subtitle: "AI Design",
    welcome: "Hello. I'm your personal design director. What website would you like to create today?",
    ...
    improve: "Improve",
    thinking: "Refining with great care...",
...
export default function CreaunaStudio() {
  ...
  const [messages, setMessages] = useState<Message[]>([
    { id: 1, role: 'ai', content: t.welcome },
  ]);
  ...
  const [previewSections, setPreviewSections] = useState<PreviewSection[]>([
    {
      id: 101,
      type: 'hero',
      html: `<div class="bg-white border border-slate-200 px-16 py-20 rounded-[3.5rem]">
        <div class="max-w-2xl">
  ...
  const quickPrompts = t.quickPrompts;
  ...
  const sendMessage = async () => {
    ...
    if (lower.includes('elegante') || lower.includes('refinada') || lower.includes('premium') || lower.includes('elegant')) {
      aiText = lang === 'es' ? "Elevado. Más espacio, tipografía refinada y detalles de lujo aplicados." : "Elevated. More space, refined typography and luxury details applied.";
      ...
      aiText = lang === 'es' ? "He añadido una sección de testimonios elegante y limpia." : "I've added an elegant and clean testimonial section.";
      newSections = [...previewSections, {
        id: Date.now(),
        type: 'testimonial',
        html: `<div class="bg-white border border-slate-200 px-12 py-14 rounded-[3.5rem]">
          <div class="max-w-lg">
            <div class="text-3xl leading-snug italic text-slate-800">“La web más bonita que he creado en mi vida. Todo el mundo me pregunta quién la hizo.”</div>
            <div class="mt-8 flex items-center gap-4">
              <div class="w-9 h-9 rounded-full bg-slate-200" />
              <div>
                <div class="font-medium">Laura Mendoza</div>
                <div class="text-xs text-slate-500">Fundadora, Atelier</div>
              </div>
            </div>
          </div>
        </div>`
      ...
      aiText = lang === 'es' ? "He mejorado el hero para que sea más impactante y emotivo." : "I've made the hero more impactful and emotional.";
      newSections = previewSections.map(sec => sec.type === 'hero' ? {
        ...sec,
    ...
    toast.success(t.successUpdate, { description: lang === 'es' ? "Cambios aplicados en tiempo real" : "Changes applied in real time" });
  };
  ...
  const changeStyle = (newStyle: 'elegante' | 'minimal' | 'moderno') => {
    ...
    setPreviewSections(prev => prev.map(section => {
      let html = section.html.replace(/bg-white|bg-slate-50/g, replacement);
      return { ...section, html };
    }));
    toast(lang === 'es' ? `Estilo aplicado: ${newStyle}` : `Style applied: ${newStyle}`);
  };

  const regenerate = () => {
    setIsThinking(true);
    setTimeout(() => {
      setPreviewSections(prev => prev.map((s, i) => ({
        ...s,
        html: s.html.replace(/rounded-\[3\.5rem\]/g, i % 2 === 0 ? 'rounded-[4.5rem]' : 'rounded-[3.5rem]')
      })));
      setIsThinking(false);
      toast.success(lang === 'es' ? "Nuevas variaciones generadas" : "New premium variations generated");
    }, 900);
  };
    ...
    setTimeout(() => {
      setPreviewSections(prev => prev.map(section => {
        if (section.id === id) {
          return {
            ...section,
            html: section.html + `<div class="mt-4 text-xs text-slate-400 tracking-widest">— REFINADO CON IA</div>`
          };
        }
        return section;
      }));
      setIsThinking(false);
      toast.success(lang === 'es' ? "Sección mejorada" : "Section improved");
    }, 700);
  };
  ...
  return (
    <div className="h-screen bg-slate-50 text-slate-900 flex flex-col overflow-hidden font-sans">
      {/* Top bar - Bright */}
      <div className="h-14 border-b border-slate-200 bg-white/95 backdrop-blur-2xl z-50 flex items-center px-6 text-sm">
        <div className="flex items-center gap-4 flex-1">
          ...
          <input 
            ...
            className="bg-transparent font-semibold tracking-tight text-xl focus:outline-none w-[260px]" 
          />
          <div className="px-3 py-px bg-slate-100 text-xs rounded-full text-slate-500">PREMIUM</div>
        </div>

        <div className="flex items-center gap-2 text-xs">
          {/* Language Switcher */}
          <div className="flex items-center border border-slate-200 rounded-2xl bg-white mr-2">
            <button 
              onClick={() => {
                setLang('es');
                toast.success("Idioma cambiado a Español");
              }} 
              className={`px-3 py-1 rounded-2xl text-xs ${lang === 'es' ? 'bg-slate-900 text-white' : ''}`}
            >
              ES
            </button>
            <button 
              ...
              className={`px-3 py-1 rounded-2xl text-xs ${lang === 'en' ? 'bg-slate-900 text-white' : ''}`}
            >
              EN
            </button>
          </div>

          <button onClick={regenerate} className="flex items-center gap-2 px-4 py-1.5 rounded-2xl border border-slate-200 hover:bg-slate-100">
            <RefreshCw className="w-3.5 h-3.5" /> {t.regenerate}
          </button>
          <button onClick={() => toast(lang === 'es' ? "Enlace copiado" : "Link copied")} className="flex items-center gap-2 px-4 py-1.5 rounded-2xl border border-slate-200 hover:bg-slate-100">
            <Share2 className="w-3.5 h-3.5" /> {t.share}
          </button>
          <button onClick={() => toast.success(lang === 'es' ? "Exportado como código limpio" : "Exported as clean code")} className="flex items-center gap-2 bg-slate-900 text-white px-5 py-1.5 rounded-2xl font-medium">
      ...
      <div className="flex flex-1 overflow-hidden pt-14">
        ...
        <div className="w-[400px] flex flex-col border-r border-slate-200 bg-white">
          ...
          {/* Messages */}
          <div className="flex-1 overflow-auto p-7 space-y-7 text-[15px]">
            <AnimatePresence>
              {messages.map((msg) => (
                <div key={msg.id} className={msg.role === 'user' ? 'flex justify-end' : ''}>
                  <div className={msg.role === 'user' 
                    ? "chat-bubble-user max-w-[83%]" 
                    : "chat-bubble-ai max-w-[92%]"}>
                    {msg.content}
                  </div>
                </div>
              ))}
          ...
          {/* Quick prompts */}
          <div className="px-7 pt-4 pb-3 border-t border-slate-100">
            <div className="text-[10px] tracking-widest text-slate-400 mb-2">{lang === 'es' ? 'SUGERENCIAS RÁPIDAS' : 'QUICK SUGGESTIONS'}</div>
            <div className="flex flex-wrap gap-2">
              {quickPrompts.map((p, i) => (
                <button key={i} onClick={() => applyQuickPrompt(p)} className="text-xs px-4 py-2 rounded-2xl border border-slate-200 hover:bg-slate-100">
                  {p}
                </button>
              ))}
            </div>
          ...
          {/* Input */}
          <div className="p-7 border-t border-slate-100 bg-slate-50">
            <div className="flex gap-2">
              ...
              <button 
                ...
                className="w-[52px] h-[52px] bg-slate-900 rounded-3xl flex items-center justify-center text-white disabled:opacity-40"
              >
          ...
          <div className="max-w-[1120px] mx-auto">
            <div className="flex justify-between items-center mb-4 px-1">
              <div className="flex items-center gap-4">
                ...
                <div className="text-xs bg-white px-3 py-px rounded-full border">{t.ready}</div>
              </div>

              <div className="flex items-center gap-3">
                <div className="flex text-xs border border-slate-200 rounded-2xl bg-white p-1">
                  {(['elegante', 'minimal', 'moderno'] as const).map((s, i) => (
                    <button key={i} onClick={() => changeStyle(s)} className={`px-5 py-1.5 rounded-[14px] transition ${style === s ? 'bg-slate-900 text-white' : 'hover:bg-slate-100'}`}>
                      {t.styles[i]}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex text-xs border border-slate-200 rounded-2xl bg-white p-1">
                  <button onClick={() => setViewMode('desktop')} className={`px-4 py-1.5 flex items-center gap-2 rounded-[14px] ${viewMode === 'desktop' ? 'bg-slate-900 text-white' : ''}`}>
                    <Monitor className="w-3.5 h-3.5" /> {lang === 'es' ? 'Escritorio' : 'Desktop'}
                  </button>
                  <button onClick={() => setViewMode('mobile')} className={`px-4 py-1.5 flex items-center gap-2 rounded-[14px] ${viewMode === 'mobile' ? 'bg-slate-900 text-white' : ''}`}>
                    <Smartphone className="w-3.5 h-3.5" /> {lang === 'es' ? 'Móvil' : 'Mobile'}
                  </button>
                </div>
              </div>
            </div>
                ...
                {previewSections.map((section) => (
                  <div key={section.id} className="group relative">
                    ...
                    <button 
                      ...
                      className="absolute -top-2 -right-2 opacity-0 group-hover:opacity-100 transition bg-white text-slate-900 text-xs px-3 py-1 rounded-full border shadow flex items-center gap-1"
                    >
